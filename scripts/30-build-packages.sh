...
#
# IGNORE_ERRORS="n m y" — чтобы один сломанный пакет (а в фидах 19.07 такие
# есть) не обрушивал сборку остальных. Именно все три буквы: в package/Makefile
# стоит $(filter n m y,$(IGNORE_ERRORS)), то есть одно лишь "m" покрывает только
# пакеты-модули. На практике сборку валил ltq-xrx500-bootcore, выбранный как =y:
# ему нужен отдельный SDK xRX500_BootCore, которого в публичных фидах нет
# (и разделу Bootcore у AX50 всё равно неоткуда взяться — его нет в разметке).
#
...
make -j"$JOBS" IGNORE_ERRORS="n m y" || {
	echo "Параллельная сборка завершилась с ошибкой, добираем однопоточно."
	make -j1 IGNORE_ERRORS="n m y" V=s || true
}
